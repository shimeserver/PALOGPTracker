...
export interface VisitDetectionResult {
  matchedLandmarkIds: string[];
  unmatchedStops: StopCluster[];
}

export function matchStopsToLandmarks(stops: StopCluster[], landmarks: Landmark[]): VisitDetectionResult {
  ...
  for (const stop of stops) {
    ...
    if (matched?.id) {
      if (!matchedLandmarkIds.includes(matched.id)) matchedLandmarkIds.push(matched.id);
    } else {
