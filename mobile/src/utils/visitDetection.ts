import { TrackPoint, Landmark } from '../types';
...
const MIN_DURATION_MS = 3 * 60 * 1000; // 3分（コンビニ等の短い立ち寄りも拾う）

// GPSポイント列から「1分以上100m圏内に滞在した」クラスタを抽出
...
export interface VisitDetectionResult {
  matchedLandmarkIds: string[];   // 既存スポットにマッチしたランドマークID
  unmatchedStops: StopCluster[];  // 未登録の立ち寄りポイント
}
...
export function matchStopsToLandmarks(
  ...
  const matchedLandmarkIds: string[] = [];
  const unmatchedStops: StopCluster[] = [];

  for (const stop of stops) {
    const matched = landmarks.find(lm =>
      haversineKm(stop.lat, stop.lng, lm.lat, lm.lng) < STOP_RADIUS_KM
    );
    if (matched?.id) {
      if (!matchedLandmarkIds.includes(matched.id)) {
        matchedLandmarkIds.push(matched.id);
      }
    } else {
  ...
  return { matchedLandmarkIds, unmatchedStops };
}
